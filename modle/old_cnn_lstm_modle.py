...
class Cnn2Lstm():
    def __init__(self):
        ...
        self.fc_out_w=tf.Variable(tf.random_normal([128,20*27],dtype=tf.float32,stddev=tf.sqrt(1/20*27)))
        self.fc_out_b=tf.Variable(tf.zeros([20*27]))
    ...
    def forward(self):
        ...
        self.out=tf.reshape(self.fc_out,[-1,20,27])
        print(self.out)

    def backward(self):
        global_step = tf.Variable(0, trainable=False)
        self.learning_rate = tf.train.exponential_decay(0.001, global_step, 1000, 0.96, staircase=True)
        ...
        for i in range(100000):
            ...
            for indexxx in range(len(o.tolist())):
                if o[indexxx].tolist() == l[indexxx].tolist():
                    ...
                    pass
            train_acc = train_count / batch_size
            if i%100==0:
            ...
            # print('第几次的输出是:{}'.format(o[0]))
                ...
                print('第{}次测试集的误差为;{}，精度为:{}'.format(i,loss1,test_acc))
                print('第{}次的误差是:{}，精度是:{}'.format(i, loss, train_acc))
